...
export default function Profile({ profile, statistics }) {
  return (
    <header className="bg-gradient-to-r from-red-900 via-rose-900 to-pink-900 p-6 rounded-xl text-white shadow-lg">
      <div className="flex flex-col md:flex-row items-start md:items-center gap-6">
        <img src={profile.photo_url} alt={profile.full_name} className="w-28 h-30 rounded-xl object-cover shadow-md" />
        ...
        <div className="flex-1">
          ...
          <div className="flex gap-4 items-center mt-3 opacity-90 text-sm">
            ...
            <span>👁️ {statistics?.views_count ?? 0} views</span>
          </div>

          <p className="mt-3 max-w-2xl">{profile.short_bio}</p>

          <a href={profile.portfolio_url || '#'} target="_blank" rel="noreferrer" className="inline-block mt-4 px-4 py-2 bg-white text-red-600 rounded-lg shadow font-semibold">
            Visit Portfolio
          </a>
        </div>
      </div>
